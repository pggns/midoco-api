...
class Hints extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \Pggns\MidocoApi\Api\Order\StructType\Hint[]
     */
    protected ?array $hint = null;
    /**
     * Constructor method for hints
     * @uses Hints::setHint()
     * @param \Pggns\MidocoApi\Api\Order\StructType\Hint[] $hint
     */
    ...
    public static function validateHintForArrayConstraintsFromSetHint(?array $values = []): string
    {
        ...
        foreach ($values as $hintsHintItem) {
            ...
            if (!$hintsHintItem instanceof \Pggns\MidocoApi\Api\Order\StructType\Hint) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The hint property can only contain items of type \Pggns\MidocoApi\Api\Order\StructType\Hint, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Pggns\MidocoApi\Api\Order\StructType\Hint[] $hint
     * @return \Pggns\MidocoApi\Api\Order\StructType\Hints
     */
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Api\Order\StructType\Hint $item
     * @return \Pggns\MidocoApi\Api\Order\StructType\Hints
     */
    public function addToHint(\Pggns\MidocoApi\Api\Order\StructType\Hint $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Api\Order\StructType\Hint) {
            throw new InvalidArgumentException(sprintf('The hint property can only contain items of type \Pggns\MidocoApi\Api\Order\StructType\Hint, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->hint[] = $item;
