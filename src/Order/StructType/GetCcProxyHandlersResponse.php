...
class GetCcProxyHandlersResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoCcProxyHandlerForArrayConstraintsFromSetMidocoCcProxyHandler(?array $values = []): string
    {
        ...
        foreach ($values as $getCcProxyHandlersResponseMidocoCcProxyHandlerItem) {
            ...
            if (!$getCcProxyHandlersResponseMidocoCcProxyHandlerItem instanceof \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCcProxyHandler property can only contain items of type \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO[] $midocoCcProxyHandler
     * @return \Pggns\MidocoApi\Api\Order\StructType\GetCcProxyHandlersResponse
     */
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO $item
     * @return \Pggns\MidocoApi\Api\Order\StructType\GetCcProxyHandlersResponse
     */
    public function addToMidocoCcProxyHandler(\Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoCcProxyHandler property can only contain items of type \Pggns\MidocoApi\Api\Order\StructType\CcproxyHandlerDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
