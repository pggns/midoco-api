...
class MidocoCateringCode extends CateringDTO
{
    ...
    public static function validateMidocoCateringDescriptionForArrayConstraintFromSetMidocoCateringDescription(?array $values = []): string
    {
        ...
        foreach ($values as $midocoCateringCodeMidocoCateringDescriptionItem) {
            ...
            if (!$midocoCateringCodeMidocoCateringDescriptionItem instanceof \Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCateringDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoCateringCode
     */
    public function addToMidocoCateringDescription(\Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoCateringDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\CateringDescriptionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
