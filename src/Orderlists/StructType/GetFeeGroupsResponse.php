...
class GetFeeGroupsResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoFeeGroupForArrayConstraintFromSetMidocoFeeGroup(?array $values = []): string
    {
        ...
        foreach ($values as $getFeeGroupsResponseMidocoFeeGroupItem) {
            ...
            if (!$getFeeGroupsResponseMidocoFeeGroupItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoFeeGroup property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetFeeGroupsResponse
     */
    public function addToMidocoFeeGroup(\Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup) {
            throw new InvalidArgumentException(sprintf('The MidocoFeeGroup property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoFeeGroup, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
