...
class GetRemarkCategoryResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoRemarkCategoryForArrayConstraintFromSetMidocoRemarkCategory(?array $values = []): string
    {
        ...
        foreach ($values as $getRemarkCategoryResponseMidocoRemarkCategoryItem) {
            ...
            if (!$getRemarkCategoryResponseMidocoRemarkCategoryItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoRemarkCategory property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetRemarkCategoryResponse
     */
    public function addToMidocoRemarkCategory(\Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory) {
            throw new InvalidArgumentException(sprintf('The MidocoRemarkCategory property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
