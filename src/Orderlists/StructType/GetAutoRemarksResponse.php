...
class GetAutoRemarksResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoSupplierAutoRemarkForArrayConstraintsFromSetMidocoSupplierAutoRemark(?array $values = []): string
    {
        ...
        foreach ($values as $getAutoRemarksResponseMidocoSupplierAutoRemarkItem) {
            ...
            if (!$getAutoRemarksResponseMidocoSupplierAutoRemarkItem instanceof \Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoSupplierAutoRemark property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetAutoRemarksResponse
     */
    public function addToMidocoSupplierAutoRemark(\Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoSupplierAutoRemark property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\SupplierAutoRemarkDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
