...
class MidocoAgencyProvisionDetails extends AbstractStructBase
{
    ...
    public static function validateAttributeForArrayConstraintsFromSetAttribute(?array $values = []): string
    {
        ...
        foreach ($values as $midocoAgencyProvisionDetailsAttributeItem) {
            ...
            if (!$midocoAgencyProvisionDetailsAttributeItem instanceof \Pggns\MidocoApi\Orderlists\StructType\Attribute_1) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Attribute property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\Attribute_1, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\Attribute_1 $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoAgencyProvisionDetails
     */
    public function addToAttribute(\Pggns\MidocoApi\Orderlists\StructType\Attribute_1 $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\Attribute_1) {
            throw new InvalidArgumentException(sprintf('The Attribute property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\Attribute_1, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Attribute[] = $item;
