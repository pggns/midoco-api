...
class GetMidocoPrintOptionResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoPrintOptionForArrayConstraintFromSetMidocoPrintOption(?array $values = []): string
    {
        ...
        foreach ($values as $getMidocoPrintOptionResponseMidocoPrintOptionItem) {
            ...
            if (!$getMidocoPrintOptionResponseMidocoPrintOptionItem instanceof \Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoPrintOption property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetMidocoPrintOptionResponse
     */
    public function addToMidocoPrintOption(\Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoPrintOption property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\PrintOptionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
