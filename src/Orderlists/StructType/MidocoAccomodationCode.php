...
class MidocoAccomodationCode extends AccomodationDTO
{
    ...
    public static function validateMidocoAccomodationDescriptionForArrayConstraintFromSetMidocoAccomodationDescription(?array $values = []): string
    {
        ...
        foreach ($values as $midocoAccomodationCodeMidocoAccomodationDescriptionItem) {
            ...
            if (!$midocoAccomodationCodeMidocoAccomodationDescriptionItem instanceof \Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoAccomodationDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoAccomodationCode
     */
    public function addToMidocoAccomodationDescription(\Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoAccomodationDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\AccomodationDescriptionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
