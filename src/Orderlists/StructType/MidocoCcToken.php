...
class MidocoCcToken extends CcTokenDTO
{
    ...
    public static function validateMidocoCcTokenAttrForArrayConstraintsFromSetMidocoCcTokenAttr(?array $values = []): string
    {
        ...
        foreach ($values as $midocoCcTokenMidocoCcTokenAttrItem) {
            ...
            if (!$midocoCcTokenMidocoCcTokenAttrItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCcTokenAttr property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoCcToken
     */
    public function addToMidocoCcTokenAttr(\Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr) {
            throw new InvalidArgumentException(sprintf('The MidocoCcTokenAttr property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoCcTokenAttr, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
