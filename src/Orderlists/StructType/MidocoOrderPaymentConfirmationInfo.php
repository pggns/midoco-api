...
class MidocoOrderPaymentConfirmationInfo extends AbstractStructBase
{
    ...
    public static function validatePaymentConfirmationInfoForArrayConstraintsFromSetPaymentConfirmationInfo(?array $values = []): string
    {
        ...
        foreach ($values as $midocoOrderPaymentConfirmationInfoPaymentConfirmationInfoItem) {
            ...
            if (!$midocoOrderPaymentConfirmationInfoPaymentConfirmationInfoItem instanceof \Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PaymentConfirmationInfo property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoOrderPaymentConfirmationInfo
     */
    public function addToPaymentConfirmationInfo(\Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo) {
            throw new InvalidArgumentException(sprintf('The PaymentConfirmationInfo property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\PaymentConfirmationInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
