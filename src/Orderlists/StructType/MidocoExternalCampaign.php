...
class MidocoExternalCampaign extends ExternalCampaignDTO
{
    ...
    public static function validateExternalCampaignAssignmentForArrayConstraintsFromSetExternalCampaignAssignment(?array $values = []): string
    {
        ...
        foreach ($values as $midocoExternalCampaignExternalCampaignAssignmentItem) {
            ...
            if (!$midocoExternalCampaignExternalCampaignAssignmentItem instanceof \Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExternalCampaignAssignment property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoExternalCampaign
     */
    public function addToExternalCampaignAssignment(\Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO) {
            throw new InvalidArgumentException(sprintf('The ExternalCampaignAssignment property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\ExternalCampaignAssignDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
