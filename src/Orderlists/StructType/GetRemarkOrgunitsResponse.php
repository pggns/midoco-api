...
class GetRemarkOrgunitsResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoRemarkOrgunitForArrayConstraintsFromSetMidocoRemarkOrgunit(?array $values = []): string
    {
        ...
        foreach ($values as $getRemarkOrgunitsResponseMidocoRemarkOrgunitItem) {
            ...
            if (!$getRemarkOrgunitsResponseMidocoRemarkOrgunitItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoRemarkOrgunit property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetRemarkOrgunitsResponse
     */
    public function addToMidocoRemarkOrgunit(\Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit) {
            throw new InvalidArgumentException(sprintf('The MidocoRemarkOrgunit property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkOrgunit, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
