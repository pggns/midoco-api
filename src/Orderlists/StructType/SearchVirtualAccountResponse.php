...
class SearchVirtualAccountResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoVirtualAccountForArrayConstraintsFromSetMidocoVirtualAccount(?array $values = []): string
    {
        ...
        foreach ($values as $searchVirtualAccountResponseMidocoVirtualAccountItem) {
            ...
            if (!$searchVirtualAccountResponseMidocoVirtualAccountItem instanceof \Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoVirtualAccount property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\SearchVirtualAccountResponse
     */
    public function addToMidocoVirtualAccount(\Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoVirtualAccount property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\VirtualAccountDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
