...
class GetHedgeCurrencyRateRequest extends AbstractStructBase
{
    ...
    public static function validateHedgeCurrencyRequestForArrayConstraintsFromSetHedgeCurrencyRequest(?array $values = []): string
    {
        ...
        foreach ($values as $getHedgeCurrencyRateRequestHedgeCurrencyRequestItem) {
            ...
            if (!$getHedgeCurrencyRateRequestHedgeCurrencyRequestItem instanceof \Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The hedgeCurrencyRequest property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetHedgeCurrencyRateRequest
     */
    public function addToHedgeCurrencyRequest(\Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest) {
            throw new InvalidArgumentException(sprintf('The hedgeCurrencyRequest property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\HedgeCurrencyRequest, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
