...
class GetSupplierDocumentsResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoSupplierDocumentForArrayConstraintFromSetMidocoSupplierDocument(?array $values = []): string
    {
        ...
        foreach ($values as $getSupplierDocumentsResponseMidocoSupplierDocumentItem) {
            ...
            if (!$getSupplierDocumentsResponseMidocoSupplierDocumentItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoSupplierDocument property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetSupplierDocumentsResponse
     */
    public function addToMidocoSupplierDocument(\Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument) {
            throw new InvalidArgumentException(sprintf('The MidocoSupplierDocument property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MidocoSupplierDocument, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
