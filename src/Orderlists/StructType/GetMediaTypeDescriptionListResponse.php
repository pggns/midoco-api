...
class GetMediaTypeDescriptionListResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoMediaTypeDescriptionForArrayConstraintsFromSetMidocoMediaTypeDescription(?array $values = []): string
    {
        ...
        foreach ($values as $getMediaTypeDescriptionListResponseMidocoMediaTypeDescriptionItem) {
            ...
            if (!$getMediaTypeDescriptionListResponseMidocoMediaTypeDescriptionItem instanceof \Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoMediaTypeDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetMediaTypeDescriptionListResponse
     */
    public function addToMidocoMediaTypeDescription(\Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoMediaTypeDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\MediaTypeDescriptionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
