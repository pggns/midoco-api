...
class GetExternalServiceLinksResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoExternalServiceLinkForArrayConstraintFromSetMidocoExternalServiceLink(?array $values = []): string
    {
        ...
        foreach ($values as $getExternalServiceLinksResponseMidocoExternalServiceLinkItem) {
            ...
            if (!$getExternalServiceLinksResponseMidocoExternalServiceLinkItem instanceof \Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoExternalServiceLink property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetExternalServiceLinksResponse
     */
    public function addToMidocoExternalServiceLink(\Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoExternalServiceLink property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\ExternalServiceLinkDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
