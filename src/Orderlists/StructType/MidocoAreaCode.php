...
class MidocoAreaCode extends AreaDTO
{
    ...
    public static function validateMidocoAreaDescriptionForArrayConstraintFromSetMidocoAreaDescription(?array $values = []): string
    {
        ...
        foreach ($values as $midocoAreaCodeMidocoAreaDescriptionItem) {
            ...
            if (!$midocoAreaCodeMidocoAreaDescriptionItem instanceof \Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoAreaDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoAreaCode
     */
    public function addToMidocoAreaDescription(\Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoAreaDescription property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\AreaDescriptionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
