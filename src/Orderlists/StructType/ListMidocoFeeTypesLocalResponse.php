...
class ListMidocoFeeTypesLocalResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoFeeTypeLocalForArrayConstraintFromSetMidocoFeeTypeLocal(?array $values = []): string
    {
        ...
        foreach ($values as $listMidocoFeeTypesLocalResponseMidocoFeeTypeLocalItem) {
            ...
            if (!$listMidocoFeeTypesLocalResponseMidocoFeeTypeLocalItem instanceof \Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoFeeTypeLocal property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\ListMidocoFeeTypesLocalResponse
     */
    public function addToMidocoFeeTypeLocal(\Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoFeeTypeLocal property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\FeeTypeLocalDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
