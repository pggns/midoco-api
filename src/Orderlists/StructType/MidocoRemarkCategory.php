...
class MidocoRemarkCategory extends RemarkCategoryDTO
{
    ...
    public static function validateMidocoRemarkCategoryDescForArrayConstraintFromSetMidocoRemarkCategoryDesc(?array $values = []): string
    {
        ...
        foreach ($values as $midocoRemarkCategoryMidocoRemarkCategoryDescItem) {
            ...
            if (!$midocoRemarkCategoryMidocoRemarkCategoryDescItem instanceof \Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoRemarkCategoryDesc property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\MidocoRemarkCategory
     */
    public function addToMidocoRemarkCategoryDesc(\Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoRemarkCategoryDesc property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\RemarkCategoryDescDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
