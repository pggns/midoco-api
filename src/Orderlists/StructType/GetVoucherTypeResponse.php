...
class GetVoucherTypeResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoVoucherTypeForArrayConstraintsFromSetMidocoVoucherType(?array $values = []): string
    {
        ...
        foreach ($values as $getVoucherTypeResponseMidocoVoucherTypeItem) {
            ...
            if (!$getVoucherTypeResponseMidocoVoucherTypeItem instanceof \Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoVoucherType property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO $item
     * @return \Pggns\MidocoApi\Orderlists\StructType\GetVoucherTypeResponse
     */
    public function addToMidocoVoucherType(\Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoVoucherType property can only contain items of type \Pggns\MidocoApi\Orderlists\StructType\VoucherTypeDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
