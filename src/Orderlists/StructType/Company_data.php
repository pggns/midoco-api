...
class Company_data extends AbstractStructBase
{
    ...
    /**
     * Set vat_type value
     * @uses \Pggns\MidocoApi\Orderlists\EnumType\VatTypeType::valueIsValid()
     * @uses \Pggns\MidocoApi\Orderlists\EnumType\VatTypeType::getValidValues()
    ...
    public function setVat_type(?string $vat_type = null): self
    {
        ...
        if (!\Pggns\MidocoApi\Orderlists\EnumType\VatTypeType::valueIsValid($vat_type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Pggns\MidocoApi\Orderlists\EnumType\VatTypeType', is_array($vat_type) ? implode(', ', $vat_type) : var_export($vat_type, true), implode(', ', \Pggns\MidocoApi\Orderlists\EnumType\VatTypeType::getValidValues())), __LINE__);
        }
