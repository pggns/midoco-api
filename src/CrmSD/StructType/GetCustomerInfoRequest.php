...
class GetCustomerInfoRequest extends AbstractStructBase
{
     ...
     * - minOccurs: 1
     * - ref: CustomerInfo
     * @var \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo[]
     */
    protected array $CustomerInfo;
    /**
     * Constructor method for GetCustomerInfoRequest
     * @uses GetCustomerInfoRequest::setCustomerInfo()
     * @param \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo[] $customerInfo
     */
     ...
     * @return \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo[]
     */
    ...
    public static function validateCustomerInfoForArrayConstraintsFromSetCustomerInfo(?array $values = []): string
    {
        ...
        foreach ($values as $getCustomerInfoRequestCustomerInfoItem) {
            ...
            if (!$getCustomerInfoRequestCustomerInfoItem instanceof \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The CustomerInfo property can only contain items of type \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo[] $customerInfo
     * @return \Pggns\MidocoApi\Api\CrmSD\StructType\GetCustomerInfoRequest
     */
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo $item
     * @return \Pggns\MidocoApi\Api\CrmSD\StructType\GetCustomerInfoRequest
     */
    public function addToCustomerInfo(\Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo) {
            throw new InvalidArgumentException(sprintf('The CustomerInfo property can only contain items of type \Pggns\MidocoApi\Api\CrmSD\StructType\CustomerInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(500)
