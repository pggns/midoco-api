...
class GetAllAssignedAdapters4OrgUnitResponse extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: MidocoAdapter
     * @var \Pggns\MidocoApi\Crm\StructType\AdapterDTO[]
     */
     ...
     * @param \Pggns\MidocoApi\Crm\StructType\AdapterDTO[] $midocoAdapter
     */
    ...
    public static function validateMidocoAdapterForArrayConstraintsFromSetMidocoAdapter(?array $values = []): string
    {
        ...
        foreach ($values as $getAllAssignedAdapters4OrgUnitResponseMidocoAdapterItem) {
            ...
            if (!$getAllAssignedAdapters4OrgUnitResponseMidocoAdapterItem instanceof \Pggns\MidocoApi\Crm\StructType\AdapterDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoAdapter property can only contain items of type \Pggns\MidocoApi\Crm\StructType\AdapterDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Crm\StructType\AdapterDTO $item
     * @return \Pggns\MidocoApi\Crm\StructType\GetAllAssignedAdapters4OrgUnitResponse
     */
    public function addToMidocoAdapter(\Pggns\MidocoApi\Crm\StructType\AdapterDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Crm\StructType\AdapterDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoAdapter property can only contain items of type \Pggns\MidocoApi\Crm\StructType\AdapterDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
