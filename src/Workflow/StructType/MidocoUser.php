...
class MidocoUser extends UserDTO
{
     ...
     * @var \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO[]
     */
    protected ?array $MidocoCrsExpedient = null;
    /**
     * Constructor method for MidocoUser
     * @uses MidocoUser::setMidocoCrsExpedient()
     * @param \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO[] $midocoCrsExpedient
     */
     ...
     * @return \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO[]
     */
    ...
    public static function validateMidocoCrsExpedientForArrayConstraintFromSetMidocoCrsExpedient(?array $values = []): string
    {
        ...
        foreach ($values as $midocoUserMidocoCrsExpedientItem) {
            ...
            if (!$midocoUserMidocoCrsExpedientItem instanceof \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCrsExpedient property can only contain items of type \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO $item
     * @return \Pggns\MidocoApi\Workflow\StructType\MidocoUser
     */
    public function addToMidocoCrsExpedient(\Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoCrsExpedient property can only contain items of type \Pggns\MidocoApi\Workflow\StructType\CrsExpedientDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
