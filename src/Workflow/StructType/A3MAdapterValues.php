...
class A3MAdapterValues extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: A3MAdapterValue
     * @var \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue[]
     */
    protected ?array $A3MAdapterValue = null;
    /**
     * Constructor method for A3MAdapterValues
     * @uses A3MAdapterValues::setA3MAdapterValue()
     * @param \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue[] $a3MAdapterValue
     */
    ...
    public static function validateA3MAdapterValueForArrayConstraintsFromSetA3MAdapterValue(?array $values = []): string
    {
        ...
        foreach ($values as $a3MAdapterValuesA3MAdapterValueItem) {
            ...
            if (!$a3MAdapterValuesA3MAdapterValueItem instanceof \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The A3MAdapterValue property can only contain items of type \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue[] $a3MAdapterValue
     * @return \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValues
     */
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue $item
     * @return \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValues
     */
    public function addToA3MAdapterValue(\Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue) {
            throw new InvalidArgumentException(sprintf('The A3MAdapterValue property can only contain items of type \Pggns\MidocoApi\Api\Workflow\StructType\A3MAdapterValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
