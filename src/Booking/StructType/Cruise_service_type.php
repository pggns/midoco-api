...
class Cruise_service_type extends AbstractStructBase
{
    ...
    {
        // validation for constraint: enumeration
        if (!\Pggns\MidocoApi\Api\Booking\EnumType\TransportType::valueIsValid($return_transport_type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Pggns\MidocoApi\Api\Booking\EnumType\TransportType', is_array($return_transport_type) ? implode(', ', $return_transport_type) : var_export($return_transport_type, true), implode(', ', \Pggns\MidocoApi\Api\Booking\EnumType\TransportType::getValidValues())), __LINE__);
        }
