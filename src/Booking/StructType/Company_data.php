...
class Company_data extends AbstractStructBase
{
     ...
     * @param string $salutation
     * @return \Pggns\MidocoApi\Booking\StructType\Company_data
     */
    ...
    public function setVat_type(?string $vat_type = null): self
    {
        ...
        if (!\Pggns\MidocoApi\Booking\EnumType\VatTypeType::valueIsValid($vat_type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Pggns\MidocoApi\Booking\EnumType\VatTypeType', is_array($vat_type) ? implode(', ', $vat_type) : var_export($vat_type, true), implode(', ', \Pggns\MidocoApi\Booking\EnumType\VatTypeType::getValidValues())), __LINE__);
        }
