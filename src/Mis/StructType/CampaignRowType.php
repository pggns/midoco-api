...
class CampaignRowType extends AbstractStructBase
{
    ...
    public static function validateColumnContentForArrayConstraintFromSetColumnContent(?array $values = []): string
    {
        ...
        foreach ($values as $campaignRowTypeColumnContentItem) {
            ...
            if (!is_string($campaignRowTypeColumnContentItem)) {
                $invalidValues[] = is_object($campaignRowTypeColumnContentItem) ? get_class($campaignRowTypeColumnContentItem) : sprintf('%s(%s)', gettype($campaignRowTypeColumnContentItem), var_export($campaignRowTypeColumnContentItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The columnContent property can only contain items of type string, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function addToColumnContent(string $item): self
    {
        // validation for constraint: itemType
        if (!is_string($item)) {
            throw new InvalidArgumentException(sprintf('The columnContent property can only contain items of type string, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->columnContent[] = $item;
