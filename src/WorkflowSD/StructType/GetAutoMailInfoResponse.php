...
class GetAutoMailInfoResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoAutoMailInfoForArrayConstraintFromSetMidocoAutoMailInfo(?array $values = []): string
    {
        ...
        foreach ($values as $getAutoMailInfoResponseMidocoAutoMailInfoItem) {
            ...
            if (!$getAutoMailInfoResponseMidocoAutoMailInfoItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoAutoMailInfo property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetAutoMailInfoResponse
     */
    public function addToMidocoAutoMailInfo(\Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoAutoMailInfo property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\AutoMailDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
