...
class MidocoUser extends UserDTO
{
    ...
    public static function validateMidocoCrsExpedientForArrayConstraintFromSetMidocoCrsExpedient(?array $values = []): string
    {
        ...
        foreach ($values as $midocoUserMidocoCrsExpedientItem) {
            ...
            if (!$midocoUserMidocoCrsExpedientItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCrsExpedient property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUser
     */
    public function addToMidocoCrsExpedient(\Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoCrsExpedient property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\CrsExpedientDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
