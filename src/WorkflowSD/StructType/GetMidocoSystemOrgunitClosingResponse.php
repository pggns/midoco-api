...
class GetMidocoSystemOrgunitClosingResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoSystemOrgunitClosingForArrayConstraintFromSetMidocoSystemOrgunitClosing(?array $values = []): string
    {
        ...
        foreach ($values as $getMidocoSystemOrgunitClosingResponseMidocoSystemOrgunitClosingItem) {
            ...
            if (!$getMidocoSystemOrgunitClosingResponseMidocoSystemOrgunitClosingItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoSystemOrgunitClosing property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetMidocoSystemOrgunitClosingResponse
     */
    public function addToMidocoSystemOrgunitClosing(\Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoSystemOrgunitClosing property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\OrgunitClosingDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
