...
class SearchMidocoAttributeLocalizationResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoSystemAttributeLocalizationForArrayConstraintFromSetMidocoSystemAttributeLocalization(?array $values = []): string
    {
        ...
        foreach ($values as $searchMidocoAttributeLocalizationResponseMidocoSystemAttributeLocalizationItem) {
            ...
            if (!$searchMidocoAttributeLocalizationResponseMidocoSystemAttributeLocalizationItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoSystemAttributeLocalization property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\SearchMidocoAttributeLocalizationResponse
     */
    public function addToMidocoSystemAttributeLocalization(\Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoSystemAttributeLocalization property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\AttributeDefinitionLocalDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
