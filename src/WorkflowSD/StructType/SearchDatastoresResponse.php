...
class SearchDatastoresResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoDatastoreForArrayConstraintFromSetMidocoDatastore(?array $values = []): string
    {
        ...
        foreach ($values as $searchDatastoresResponseMidocoDatastoreItem) {
            ...
            if (!$searchDatastoresResponseMidocoDatastoreItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoDatastore property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\SearchDatastoresResponse
     */
    public function addToMidocoDatastore(\Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoDatastore property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\DatastoreDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
