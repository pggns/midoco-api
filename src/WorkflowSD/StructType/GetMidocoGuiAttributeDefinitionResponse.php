...
class GetMidocoGuiAttributeDefinitionResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoGuiAttributeDefinitionForArrayConstraintFromSetMidocoGuiAttributeDefinition(?array $values = []): string
    {
        ...
        foreach ($values as $getMidocoGuiAttributeDefinitionResponseMidocoGuiAttributeDefinitionItem) {
            ...
            if (!$getMidocoGuiAttributeDefinitionResponseMidocoGuiAttributeDefinitionItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoGuiAttributeDefinition property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetMidocoGuiAttributeDefinitionResponse
     */
    public function addToMidocoGuiAttributeDefinition(\Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO) {
            throw new InvalidArgumentException(sprintf('The MidocoGuiAttributeDefinition property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\GuiAttributeDefinitionDTO, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
