...
class GetAssignedSystemRulesToOrgunitResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoUnitRulesetForArrayConstraintFromSetMidocoUnitRuleset(?array $values = []): string
    {
        ...
        foreach ($values as $getAssignedSystemRulesToOrgunitResponseMidocoUnitRulesetItem) {
            ...
            if (!$getAssignedSystemRulesToOrgunitResponseMidocoUnitRulesetItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoUnitRuleset property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetAssignedSystemRulesToOrgunitResponse
     */
    public function addToMidocoUnitRuleset(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset) {
            throw new InvalidArgumentException(sprintf('The MidocoUnitRuleset property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUnitRuleset, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
