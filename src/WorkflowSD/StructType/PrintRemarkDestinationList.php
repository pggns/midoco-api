...
class PrintRemarkDestinationList extends AbstractStructBase
{
    ...
    public static function validateMidocoPrintRemarkDestinationForArrayConstraintFromSetMidocoPrintRemarkDestination(?array $values = []): string
    {
        ...
        foreach ($values as $printRemarkDestinationListMidocoPrintRemarkDestinationItem) {
            ...
            if (!$printRemarkDestinationListMidocoPrintRemarkDestinationItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoPrintRemarkDestination property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\PrintRemarkDestinationList
     */
    public function addToMidocoPrintRemarkDestination(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination) {
            throw new InvalidArgumentException(sprintf('The MidocoPrintRemarkDestination property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoPrintRemarkDestination, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
