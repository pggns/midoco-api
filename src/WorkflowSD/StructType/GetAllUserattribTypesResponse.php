...
class GetAllUserattribTypesResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoUserattribTypeForArrayConstraintFromSetMidocoUserattribType(?array $values = []): string
    {
        ...
        foreach ($values as $getAllUserattribTypesResponseMidocoUserattribTypeItem) {
            ...
            if (!$getAllUserattribTypesResponseMidocoUserattribTypeItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoUserattribType property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetAllUserattribTypesResponse
     */
    public function addToMidocoUserattribType(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType) {
            throw new InvalidArgumentException(sprintf('The MidocoUserattribType property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoUserattribType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
