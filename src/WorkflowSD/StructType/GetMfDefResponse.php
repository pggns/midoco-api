...
class GetMfDefResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoMfDefForArrayConstraintFromSetMidocoMfDef(?array $values = []): string
    {
        ...
        foreach ($values as $getMfDefResponseMidocoMfDefItem) {
            ...
            if (!$getMfDefResponseMidocoMfDefItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoMfDef property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetMfDefResponse
     */
    public function addToMidocoMfDef(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef) {
            throw new InvalidArgumentException(sprintf('The MidocoMfDef property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoMfDef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
