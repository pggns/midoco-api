...
class GetDocumentNaturesResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoDocumentNatureForArrayConstraintFromSetMidocoDocumentNature(?array $values = []): string
    {
        ...
        foreach ($values as $getDocumentNaturesResponseMidocoDocumentNatureItem) {
            ...
            if (!$getDocumentNaturesResponseMidocoDocumentNatureItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoDocumentNature property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetDocumentNaturesResponse
     */
    public function addToMidocoDocumentNature(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType) {
            throw new InvalidArgumentException(sprintf('The MidocoDocumentNature property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
