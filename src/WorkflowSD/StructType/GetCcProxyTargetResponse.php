...
class GetCcProxyTargetResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoCcProxyTargetForArrayConstraintFromSetMidocoCcProxyTarget(?array $values = []): string
    {
        ...
        foreach ($values as $getCcProxyTargetResponseMidocoCcProxyTargetItem) {
            ...
            if (!$getCcProxyTargetResponseMidocoCcProxyTargetItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoCcProxyTarget property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetCcProxyTargetResponse
     */
    public function addToMidocoCcProxyTarget(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget) {
            throw new InvalidArgumentException(sprintf('The MidocoCcProxyTarget property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoCcProxyTarget, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
