...
class GetRolesFromProcessRoleAssignResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoProcessRoleAssignForArrayConstraintFromSetMidocoProcessRoleAssign(?array $values = []): string
    {
        ...
        foreach ($values as $getRolesFromProcessRoleAssignResponseMidocoProcessRoleAssignItem) {
            ...
            if (!$getRolesFromProcessRoleAssignResponseMidocoProcessRoleAssignItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoProcessRoleAssign property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetRolesFromProcessRoleAssignResponse
     */
    public function addToMidocoProcessRoleAssign(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign) {
            throw new InvalidArgumentException(sprintf('The MidocoProcessRoleAssign property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoProcessRoleAssign, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
