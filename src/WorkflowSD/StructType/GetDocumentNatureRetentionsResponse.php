...
class GetDocumentNatureRetentionsResponse extends AbstractStructBase
{
    ...
    public static function validateMidocoDocumentNatureRetentionForArrayConstraintFromSetMidocoDocumentNatureRetention(?array $values = []): string
    {
        ...
        foreach ($values as $getDocumentNatureRetentionsResponseMidocoDocumentNatureRetentionItem) {
            ...
            if (!$getDocumentNatureRetentionsResponseMidocoDocumentNatureRetentionItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MidocoDocumentNatureRetention property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\GetDocumentNatureRetentionsResponse
     */
    public function addToMidocoDocumentNatureRetention(\Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType) {
            throw new InvalidArgumentException(sprintf('The MidocoDocumentNatureRetention property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\MidocoDocumentNatureRetentionType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
