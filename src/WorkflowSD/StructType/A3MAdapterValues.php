...
class A3MAdapterValues extends AbstractStructBase
{
    ...
    public static function validateA3MAdapterValueForArrayConstraintFromSetA3MAdapterValue(?array $values = []): string
    {
        ...
        foreach ($values as $a3MAdapterValuesA3MAdapterValueItem) {
            ...
            if (!$a3MAdapterValuesA3MAdapterValueItem instanceof \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The A3MAdapterValue property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue $item
     * @return \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValues
     */
    public function addToA3MAdapterValue(\Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue) {
            throw new InvalidArgumentException(sprintf('The A3MAdapterValue property can only contain items of type \Pggns\MidocoApi\WorkflowSD\StructType\A3MAdapterValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
